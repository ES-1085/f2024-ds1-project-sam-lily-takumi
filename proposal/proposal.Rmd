...
sept_kpis_cleaned$goal_percent <- sept_kpis_cleaned$`% of Goal`
sept_kpis_cleaned <- sept_kpis_cleaned[-c(1,11), ]
...
# STEP 2: Calculate cumulative total per Assumption
kpis_long <- kpis_long %>%
  group_by(Assumptions) %>%
  arrange(Month) %>%
  mutate(Cumulative = cumsum(Monthly_Total))

# STEP 3: Calculate estimated Goal from YTD and % of Goal
kpis_long <- kpis_long %>%
  left_join(
    sept_kpis_cleaned %>%
      mutate(
...
# STEP 4: Plot the cumulative progress
ggplot(kpis_long, aes(x = Month, y = Cumulative)) +
  geom_line(color = "steelblue", size = 1) +
  geom_point(size = 1.5) +
  geom_hline(aes(yintercept = Goal), linetype = "dashed", color = "darkred") +
  facet_wrap(~ Assumptions, scales = "free_y") +
  labs(
    title = "Cumulative Monthly Progress Toward Goal",
    x = "Month",
    y = "Cumulative Total"
  ) +
  theme_minimal()
